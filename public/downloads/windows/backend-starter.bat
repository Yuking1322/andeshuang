...
echo 生成时间: 2026/3/13 22:09:17 >> "%LOGFILE%"
echo ===================================== >> "%LOGFILE%"
echo.
echo =====================================
echo      安的爽 - 开发环境一键安装
...
echo.
echo [安装] OpenJDK 17
echo [安装] OpenJDK 17 >> "%LOGFILE%"
choco install openjdk17 -y --no-progress >> "%LOGFILE%" 2>&1
if %errorlevel% equ 0 (
  echo [成功] OpenJDK 17
  echo [成功] OpenJDK 17 >> "%LOGFILE%"
) else (
  echo [失败] OpenJDK 17
  echo [失败] OpenJDK 17 >> "%LOGFILE%"
  set /a FAILED_COUNT+=1
)
echo.
